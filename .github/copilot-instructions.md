# Smart Video Communications - Project Rules
...
The project follows a structured documentation flow. Always reference these documents when making architectural or design decisions:

1. **docs/01_Requirements.md** - Functional and non-functional requirements, SLOs
2. **docs/02_System-Architecture.md** - High-level architecture, WebRTC fundamentals
...
- **User Service**: Auth, Profiles, Tenant Management (.NET 10 API, SQL)
...
- **Media Controller**: SFU allocation, Load Balancing (Node.js/Go)
- **Recording Service**: Stream ingestion, Transcoding, Uploads (Python/FFmpeg, Blob)
- **Notification Service**: Email, Push, SMS (Azure Functions)
- **Analytics Service**: Telemetry ingestion, Aggregation (Spark/Flink, Kafka)
...
2. **Review architecture** in `docs/02_System-Architecture.md`
3. **Follow detailed design** in `docs/03_Detailed-Design.md`
4. **Use tech stack** from `docs/04_Implementation-Plan.md`

### When Creating APIs

- Define endpoints per `docs/03_Detailed-Design.md` Part A
...
- Follow data models in `docs/03_Detailed-Design.md` Part F-A
...
- Follow WebSocket events in `docs/03_Detailed-Design.md` Part F-B
...
- Does this meet the requirements in `docs/01_Requirements.md`?
- Does this follow the architecture in `docs/02_System-Architecture.md`?
- Are there detailed specs in `docs/03_Detailed-Design.md`?
- Is this using the tech stack from `docs/04_Implementation-Plan.md`?
